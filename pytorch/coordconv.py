...
class AddCoords(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        if self.rank == 1:
            ...
            xx_range = torch.arange(dim_x, dtype=torch.int32, device=device)
            ...
            xx_ones = torch.ones([1, 1, 1, dim_x], dtype=torch.float, device=device)
            yy_ones = torch.ones([1, 1, 1, dim_y], dtype=torch.float, device=device)
            ...
            zz_ones = torch.ones([1, 1, 1, 1, dim_z], dtype=torch.int32, device=device)

            xy_range = torch.arange(dim_y, dtype=torch.int32, device=device)
            xy_range = xy_range[None, None, None, :, None]

            yz_range = torch.arange(dim_z, dtype=torch.int32, device=device)
            yz_range = yz_range[None, None, None, :, None]

            zx_range = torch.arange(dim_x, dtype=torch.int32, device=device)
